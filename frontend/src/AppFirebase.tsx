...
export default function App() {
  ...
  const cargarTareas = async () => {
    try {
      setCargando(true);
      const q = query(collection(db, 'tareas'), orderBy('fecha'), orderBy('hora'));
      ...
      alert('Error al cargar tareas. Verifica tu conexión.');
    } finally {
      setCargando(false);
    }
  };
  ...
  const crearTarea = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'tareas'), {
        ...
        hora: nuevaTarea.hora || '',
        completada: false,
        prioridad: nuevaTarea.prioridad,
        recordatorio: nuevaTarea.recordatorio
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600 hidden sm:block">online • Firebase</p>
              </div>
            </div>
            <button
              onClick={() => {
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        {cargando && (
          <div className="fixed top-20 right-4 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg text-sm">
            Cargando...
          </div>
        )}
